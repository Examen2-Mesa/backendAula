...
class SesionAsistencia(Base):
    ...
    radio_permitido_metros = Column(Integer, default=100)  # Radio en metros (100m por defecto)
    permite_asistencia_tardia = Column(Boolean, default=True)  # Si permite marcar después de inicio
    minutos_tolerancia = Column(Integer, default=15)  # Minutos de tolerancia para llegada tardía
    ...
    def esta_activa(self):
        ...
        tiempo_limite = self.fecha_inicio.replace(
            minute=self.fecha_inicio.minute + self.minutos_tolerancia
        )
